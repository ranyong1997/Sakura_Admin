...
from app.db.models import Post, Category, Comment, post_category
from app.db.schemas.post import PostIn, PostInUpdate
from app.utils.elastic import es_update_doc, es_delete_doc, es_create_doc
...
class PostDAL:
    ...
    async def get_categories_by_ids(self, ids: List[int]):
        stmt = self(Category).where(Category.id.in_(ids))
    ...
    async def get_by_id(self, record_id: int, is_published=None, can_comment=None, reviewed=None):
        ...
        if reviewed is not None:
            stmt = stmt.options(with_loader_criteria(Comment, Comment.reviewed == reviewed))
        q = await self.db_session.execute(stmt)
