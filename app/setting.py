...
class Settings(BaseSettings):
    ...
    DB_USER = dotenv_config.get('DB_USER', 'fastapivueblog')  # 数据库用户名
    DB_PASSWORD = dotenv_config.get('DB_PASSWORD', 'fastapivueblog12306')  # 数据库密码
    DB_NAME = dotenv_config.get('DB_NAME', 'fastapivueblog')  # 数据库名称

    DATABASE_URI = f'mysql+pymysql://{DB_USER}:{DB_PASSWORD}@{DB_HOST}:{DB_PORT}'  # 数据库连接字符串
    SQLALCHEMY_DATABASE_URI: str = f'mysql+aiomysql://{DB_USER}:{DB_PASSWORD}@{DB_HOST}:{DB_PORT}/{DB_NAME}'  # 数据库连接字符串
    ...
    SECRET_KEY: str = secrets.token_urlafe(32)  # 秘钥
