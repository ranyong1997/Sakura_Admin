...
class ServiceRoll(ServiceBase):
    ...
    def invalid_args_process(self, error_type: ServiceFailure, args: list = None):
        ...
        self.bot.send_rich_message_to_chat(self.open_chat_id, title=self.MESSAGE_ROLL_ERROR_RESULT_TITLE,
    ...
    def __process_roll__(self, ) -> bool:
        ...
        while len(member_names) < self.roll_result_size:
            ...
            # roll 到免打扰，跳过
            if self.LABEL in self.no_disturb_dict.keys() and self.open_chat_id in self.no_disturb_dict[
                self.LABEL].keys():
