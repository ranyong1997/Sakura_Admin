...
from webapi.app.crud import connect, Mapper
...
class DashboardDao(Mapper):
    ...
    async def get_report_statistics(cls, start: datetime, end: datetime, session: AsyncSession = None):
        ...
        sql = cls.create_sql(SakuraReport, start, end, field="start_at")
        ...
        for item in data.scalars().all():
            ...
            result[idx[date]]["failed"] = result[idx[date]].get("failed", 0) + item.success_count
            result[idx[date]]["error"] = result[idx[date]].get("error", 0) + item.success_count
            result[idx[date]]["skip"] = result[idx[date]].get("skip", 0) + item.success_count
