...
from webapi.app.enums.CaseStatusEnum import CaseStatus
from webapi.app.enums.ConstructorEnum import ConstructorType
from webapi.app.enums.RequestBodyEnum import BodyType
from webapi.app.enums.RequestTypeEnum import RequestType
from webapi.app.excpetions.convert.GenerateException import GenerateException
from webapi.app.schema.constructor import ConstructorForm
from webapi.app.schema.request import RequestInfo
from webapi.app.schema.testcase_schema import TestCaseForm
...
class CaseGenerator(object):
    ...
    def generate_constructors(requests: List[RequestInfo]) -> List[ConstructorForm]:
        ...
        for r in range(len(requests) - 1):
            ...
            constructor_json = json.dumps(dict(
                ...
                body_type=CaseGenerator.get_body_type(requests[r].request_headers)), ensure_ascii=False)
    ...
    def split_body(request: RequestInfo, ans: dict, var_name: str = ""):
        if request.body:
            try:
                ...
                raise GenerateException(f"解析接口body变量出错:{str(e)}") from e
    ...
    def split_headers(request: RequestInfo, ans: dict, var_name: str = ""):
        try:
            ...
            raise GenerateException(f"解析接口headers变量出错:{str(e)}") from e
    ...
    def replace_body(request: RequestInfo, ans: dict, replaced: list):
        if request.body:
            try:
                ...
                for v in var_type:
                    result = result.replacef('"{v}"', f"{v}")
                request.body = result
            except JSONDecodeError:
                pass
            except Exception as e:
                logger.error(f"转换body变量失败:{str(e)}")
