...
from webapi.app.middleware.oss import OssFile
...
class QiniuOss(OssFile):
    ...
    def __init__(self, access_key_id: str, access_key_secret: str, bucket: str):
        ...
        self.bucket = bucket
        self.bucket_manager = BucketManager(self.bucket)
    ...
    async def download_file(self, filepath, base_path: str = None):
        ...
        base_url = f'{Config.OSS_URL}/{self.bucket}/{filepath}'
        url = self.auth.private_download_url(base_url, expires=3600 * 24 * 365)
        content, real_name = await self.download_file(key, url)
    ...
    async def get_file_object(self, filepath):
        ...
        if exists is None:
            raise Exception("文件不存在")
        async with aiohttp.ClientSession() as session:
            basic_url = f'{Config.OSS_URL}/{self.bucket}/{filepath}'
