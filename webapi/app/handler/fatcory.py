...
from webapi.app.handler.encoder import jsonable_encoder
...
class SakuraResponse(object):
    ...
    def parse_sql_result(data: list):
        ...
        return columns, [SakuraResponse.json_serialize(obj) for obj in data]

    @staticmethod
    def model_to_list(data: list, *ignore: str):
        return [SakuraResponse.model_to_dict(x, *ignore) for x in data]
    ...
    def success(data=None, code=0, msg="操作成功", exclude=()):
        return SakuraResponse.encode_json(dict(code=code, msg=msg, data=data), *exclude)
